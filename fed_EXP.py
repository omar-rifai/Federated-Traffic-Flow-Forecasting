...
def experiment_dataset(cluster_size,df,pathname="/projets/fedvilagil/experiment/clusterS"):
    ...
    with open('{}{}.pkl'.format(pathname,cluster_size), 'wb') as f:
        pickle.dump(cluster_dict, f)

def experiment_dataset_subgraph(cluster_size,df,pathname="/projets/fedvilagil/experiment/clusterGsize"):
    cluster_dict={"size":cluster_size}
...
experiment_dataset(1,PeMS,"/projets/fedvilagil/experiment/fedS1")
experiment_dataset_subgraph(1,PeMS,"/projets/fedvilagil/experiment/fedG1")

# load the experiment datasets from pickle object 
with open('/projets/fedvilagil/experiment/clusterS{}.pkl'.format(i), 'rb') as f:
...
with open('/projets/fedvilagil/experiment/clusterS{}.pkl'.format(i), 'wb') as f:
...
for i in [1,5,10,15]:
# load the experiment datasets from pickle object 
    with open('/projets/fedvilagil/experiment/clusterGsize{}.pkl'.format(i), 'rb') as f:
        my_dict = pickle.load(f)
        # iterate on number of cluster 100-i+1
        for j in range(100-i+1):
            train = my_dict[j]["train"]
            ...
            model = train_model(model,train, val)
            my_dict[j]["model"]=copy.deepcopy(model)
    with open('/projets/fedvilagil/experiment/clusterGsize{}.pkl'.format(i), 'wb') as f:
        pickle.dump(my_dict, f)
